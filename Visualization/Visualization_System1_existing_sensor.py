...
for i in range(0,Existingsensor):
    ...
    #p.circle(x=xn, y=yn, line_color="blue",size=15,fill_color="black", alpha=0.7, legend_label="Existing sensor locations")
...
# Set the label_text_font_size for the legend
p.legend.label_text_font_size = "15pt"
items = [("Existing sensor locations", [p.circle(line_color="blue",size=15,fill_color="black", alpha=0.7)]),
                      ("Covered nodes", [p.circle(line_color="red", size=10, fill_color="pink", alpha=0.7)]),
                      ("Manhole locations", [p.circle(line_color="red",size=10,fill_color="yellow", alpha=0.5)]), 
...
#legend = Legend(label_text_font_size="20pt", location='bottom_right')
#items = [LegendItem(label="Existing sensor locations", renderers=[p.circle(line_color="blue",size=15,fill_color="black", alpha=0.7)]),
...
#p.add_layout(legend)
p.text(x = min(xs) + 0.15*((min(xs)+max(xs))/2-min(xs)), y = min(ys), text=["Total length cover = 2,068.8m"], text_color="blue", text_font_size="20pt")
...
ID = np.array([str(ExistingsensorLoc['Index'][i])])
