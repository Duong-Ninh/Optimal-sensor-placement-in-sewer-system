...
toolbox.register("individual", tools.initRepeat, creator.Individual,toolbox.attr_bool, 189)
...
def evalFunc(individual):
    ...
    for i in range (len(individual)):  #range (0, 189)
        if (individual[i]==1):
            tmp=DownStreamDependentDB.iloc[:,i].dropna()
...
#count duplicates of coveredNodesbyManhole_4GA and nonDup_coveredNodesbyManhole
    countObserved_4GA=[] 
    for i in range (len(nonDup_coveredNodesbyManhole)):    
    ...
    fit=0
    fitness=0;
    for i in range(len(countObserved_4GA)):   
...
## Run GA
## Don't need to run anymore after saving result to System2_network.xlsx
...
out.to_csv('Ga_out_minimise_sensors_System2_70%.csv')
