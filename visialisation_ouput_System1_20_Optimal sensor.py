...
minSensor=len(GAresult.loc[GAresult['20SensorLoc']!=0])
...
sensorLoc=GAresult.loc[GAresult['20SensorLoc']!=0]
...
for i in range(0,minSensor):
    if sensorLoc['20SensorLoc'][i] in downstreamDependent:
        td = downstreamDependent[sensorLoc['20SensorLoc'][i]].dropna()
...
for i in range(0,minSensor):
    xn=NodeCoord.loc[NodeCoord['Node']==sensorLoc['20SensorLoc'][i]]['X-Coord'].values
    yn=NodeCoord.loc[NodeCoord['Node']==sensorLoc['20SensorLoc'][i]]['Y-Coord'].values
    if sensorLoc['20SensorLoc'][i] in downstreamDependent:
        td = downstreamDependent[sensorLoc['20SensorLoc'][i]].dropna()
        tmp1 = findlinkupstream(td, NodeCoord) 
        p.circle(tmp1[0], tmp1[1], line_color="red", size=10, fill_color="pink", alpha=0.7)       
    else:
        print(f"KeyError: {sensorLoc['20SensorLoc'][i]} not found in downstreamDependent dictionary") 
...
p.text(x = min(xs) + 0.2*((min(xs)+max(xs))/2-min(xs)), y = min(ys), text=["Total length cover = 5,062.2m"], text_color="blue", text_font_size="18pt")
