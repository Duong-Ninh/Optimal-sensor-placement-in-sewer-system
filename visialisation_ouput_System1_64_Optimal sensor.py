...
# p.text(x = min(xs) + 0.2*((min(xs)+max(xs))/2-min(xs)), y = min(ys), text=["Total length cover = 3,827.1 m"], text_color="blue", text_font_size="20pt")
p.text(x = max(xs) - 0.8*((min(xs)+max(xs))/2-min(xs)), y = max(ys), text=["System I - A flat system"], 
