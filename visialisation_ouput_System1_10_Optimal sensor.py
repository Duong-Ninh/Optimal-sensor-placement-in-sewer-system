...
minSensor=len(GAresult.loc[GAresult['10SensorLoc']!=0])
...
sensorLoc=GAresult.loc[GAresult['10SensorLoc']!=0]
...
nodes = ManholesID['Index'].astype(str).values
source = ColumnDataSource(data={'x':xm,'y':ym, 'node': nodes})
...
#p.circle(x=xm, y=ym, line_color="red",size=10,fill_color="yellow", alpha=0.5)#, legend_label="Manhole locations")  
...
for i in range(0,minSensor):
    if sensorLoc['10SensorLoc'][i] in downstreamDependent:
        td = downstreamDependent[sensorLoc['10SensorLoc'][i]].dropna()
...
for i in range(0,minSensor):
    xn=NodeCoord.loc[NodeCoord['Node']==sensorLoc['10SensorLoc'][i]]['X-Coord'].values
    yn=NodeCoord.loc[NodeCoord['Node']==sensorLoc['10SensorLoc'][i]]['Y-Coord'].values
    if sensorLoc['10SensorLoc'][i] in downstreamDependent:
        td = downstreamDependent[sensorLoc['10SensorLoc'][i]].dropna()
        tmp1 = findlinkupstream(td, NodeCoord) 
        p.circle(tmp1[0], tmp1[1], line_color="red", size=10, fill_color="pink", alpha=0.7)       
    else:
        print(f"KeyError: {sensorLoc['10SensorLoc'][i]} not found in downstreamDependent dictionary") 
...
# plot labels for sensor nodes
    ...
    labels = LabelSet(x='x', y='y', text='ID', source=source1, x_offset=5, y_offset=5, render_mode='canvas', text_font_size='20pt')
    p.add_layout(labels)


# plot a text of length cover
p.text(x = min(xs) + 0.2*((min(xs)+max(xs))/2-min(xs)), y = min(ys), text=["Total length cover = 2,954.5m"], text_color="blue", text_font_size="20pt")

#p.text(x = max(xs) - 0.8*((min(xs)+max(xs))/2-min(xs)), y = max(ys), text=["System I - A flat system"],text_color="Black", text_font_size="15pt", text_font_style = "bold")


legend = Legend(label_text_font_size="14pt", location='bottom_right')
...
ID = np.array([str(sensorLoc['Index'][i])])
